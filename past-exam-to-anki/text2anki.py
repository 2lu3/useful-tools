...
def split_question_answer(text: str, delimiter: str = "解答") -> tuple[str, str]:
    ...
    *delimiter* が見つからなかった場合は、全文を問題とし、解答は空文字列とする。
    ...
    answer_start_idx = next((i for i, line in enumerate(lines) if delimiter in line), None)
...
if __name__ == "__main__":
    ...
    for path in txt_files:
        ...
        try:
            ...
            logger.warning(f"UTF-8 デコードに失敗: {path.name} -> bytes デコードで再試行")
        ...
        logger.debug(f"抽出完了: {path.name} -> 問題 {len(question)} 文字 / 解答 {len(answer)} 文字")

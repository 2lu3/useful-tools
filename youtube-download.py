...
command_option = 'youtube-dl -f bestvideo[ext=mp4]+bestaudio[ext=m4a] -o ' + out_dir + '%(title)s'
...
for i in range(round(video_number / video_per_channel)):
  ...
  command = command_option + ' ' + url + ' --playlist-start ' + str(start) + ' --playlist-end ' + str(end)
  print(command)
  if end == video_number:
